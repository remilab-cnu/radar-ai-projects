...
이 프로젝트에서는 1D CNN이 손상된 beat signal로부터 불균형 파라미터 4개를 직접 추정하고,
해석적 역변환으로 원신호를 복원한다.
...
```
p06_iq_imbalance/
  generate_data.py   # I/Q imbalance 데이터셋 생성
  model.py           # IQImbalanceCNN 1D CNN 정의
  train.py           # 학습 + Gram-Schmidt baseline 비교
