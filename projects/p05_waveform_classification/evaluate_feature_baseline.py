#!/usr/bin/env python3
"""P05 handcrafted-feature baselines for waveform classification."""
...
def main() -> None:
    ...
    x_train, y_train = load_features(train_data_dir / "waveform_train.h5")
    x_val, y_val = load_features(train_data_dir / "waveform_val.h5")
    x_eval, y_eval = load_features(eval_data_dir / f"waveform_{args.eval_split}.h5")
    ...
    payload = {
        ...
        "eval_confusion_matrix": confusion_matrix(y_eval, eval_pred, labels=list(range(len(WAVEFORM_CLASSES)))).tolist(),
    }
    out = Path(args.out)
