#!/usr/bin/env python3
"""P05 — Train a lightweight radar waveform classifier."""
...
from shared.waveform_library import SCHEMA_VERSION, WAVEFORM_CLASSES


def assert_current_schema(path: Path) -> None:
    with h5py.File(path, "r") as f:
        version = int(f["schema_version"][0]) if "schema_version" in f else -1
        required = ["waveform_class_names", "snr_db", "features", "matlab_reference_url", "stft_window"]
    ...
    if version != SCHEMA_VERSION or missing:
        raise ValueError(
            ...
            "regenerate P05 data before training/eval."
        )


def load_waveform_dataset(path: Path) -> TensorDataset:
...
def evaluate(model: torch.nn.Module, dataset: TensorDataset, device: str, max_samples: int = 3000) -> dict:
    ...
    y_prob = torch.cat(probs).numpy()[:max_samples]
    report = classification_report(y_true, y_pred, labels=list(range(len(WAVEFORM_CLASSES))))
    report.update({
        "class_names": list(WAVEFORM_CLASSES),
        "n_eval": int(len(y_true)),
    })
    if y_prob.shape[1] == 2:
        report["positive_probability_mean"] = float(np.mean(y_prob[:, 1]))
    return report


def main() -> None:
    p = base_parser("P05: Lightweight Radar Waveform Classification")
    ...
    train_path = data_dir / "waveform_train.h5"
    val_path = data_dir / "waveform_val.h5"
    test_path = data_dir / "waveform_test.h5"
    for path in [train_path, val_path, test_path]:
        if not path.exists():
            raise FileNotFoundError(f"{path} not found. Use --generate first.")

    train_ds = load_waveform_dataset(train_path)
    val_ds = load_waveform_dataset(val_path)
    test_ds = load_waveform_dataset(test_path)
    print(f"  Train: {len(train_ds)}, Val: {len(val_ds)}, Test: {len(test_ds)}")
    print(f"  Classes ({len(WAVEFORM_CLASSES)}): {list(WAVEFORM_CLASSES)}")

    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = make_waveform_model(
    ...
    if args.eval_only:
        ...
        results.update({"model": "tiny_waveform_cnn", "base_ch": args.base_ch})
